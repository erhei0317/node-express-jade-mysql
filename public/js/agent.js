...
$(function(){
    ...
    $('#add').click(function() {       //保存添加的内容
        ...
        $.ajax({
            ...
            if(data.code == 200) {
                showDialogOneMsg('添加成功','添加代理成功，点击‘继续添加’继续添加代理，点击‘返回’返回上一级','继续添加','返回', '/levels/list');
            } else if(data.code == 2) {
                showDialogTwoMsg('添加失败','当前代理已存在','确定');
            } else if(data.code == 3) {
    ...
    $('#edit').click(function() {
        ...
        $.ajax({
            ...
            if(data.code == 200) {
                ...
                showDialogTwoMsg('修改失败','当前代理已存在','确定');
            } else if(data.code == 3) {
                showWarnMsg('级别名称不能为空');
            } else{
    ...
    console.log(11)
    $("#productId").click(function(){       //根据产品id筛选代理等级
        alert(1)
